...
function find_Editor_group_replay_Message()
{
    ...
    if(!message_editor_group.tobox || message_editor_group.tobox.length == 0)
    {
		...
		//fill the tobox content, the content is created by Raymond Fu,
		//the code like this:
		/*
		var objName = "webMessageEmails"; 
		var node = doc.getElementById("pagelet_web_messenger"); 
		var emailObj = FnGlobalObject.getObject(objName, node);
		*/
		//FnGlobalObject is global object within the window of browser.xul
		// if the function fail, please contact Raymond Fu
		self.port.emit(c_msg_name_get_recipient_for_reply_message_page ,"");
    }
}
...
self.port.on(c_msg_name_get_recipient_for_reply_message_page_feedback , function(data) {

	WEB_CMM.log( " on "+ c_msg_name_get_recipient_for_reply_message_page_feedback +" data:"   + data);
	var data_obj = JSON.parse(data) ;// the format is [{"email":""qq@ww.cc},{"email":""ww@ww.cc}] 
	var recipeints= [];
	for(var i = 0; i< data_obj.length; i++)
	{
		recipeints.push(data_obj[i].email);
	}
	var recipeints_str = recipeints.join(";");
	var nodestr = "<span>"+recipeints_str+"</sapen>"
	//message_editor_group.tobox();
	$(nodestr).hide().appendTo(message_editor_group.tobox  );
});
self.port.on("getEditors", function(url_spec) {
    ...
    if( is_my_page(url_spec) )
    {
        ...
        {
            WEB_CMM.log("** facebook message -reply match the url ***" + url_spec );
