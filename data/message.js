...
function find_Editor_group_replay_Message()
{
 ...
 // find editor
  ...
  if(!reply_message_editor_group.tobox)
  {
	  ...
	  self.port.emit("getEditor", "find to_boxe count is -- create new  and hide : " + " " +a_id );
  }
}
...
{
  // editor
	var role_name = PROJECT_NAME_PREFIX + INTEGRATING_ROLE_NAME + reply_message_editor_group.id;
	if( reply_message_editor_group.editor)
	{
		self.port.emit("getEditor- clean_Editor_group ", "editor_group.editor  " + reply_message_editor_group.editor);
	...
	if( reply_message_editor_group.tobox)
	{
	   ...
	   self.port.emit("getEditor", "tobox$ remove +  " );
...
self.port.on("getEditors", function(url_spec) {
	...
	//not "https://www.facebook.com/messages/new" but for "https://www.facebook.com/messages/xxxxxx"
	var reg_URL = /^https:\/\/www.facebook.com\/messages\/(?!new)./ ;
	var match = reg_URL.test( url_spec );
	if(match)
	{

		console.log("** match the url ***" + url_spec);
