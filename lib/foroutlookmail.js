...
//const 
//pref :
const c_pref_fn_conversation_enable = "fn-conversation-outlook-enable";
...
{
	aworker.port.emit(c_msg_name_get_outlook_composer, aworker.url);
}
////]
...
{
	try{
	aworker.port.emit(c_msg_name_status_outlook_action, enabled);
	}
	catch(err){}
}
...
{
	log("on_msg_fn_toggle_options_persist . outlookmail:" + is_start )
...
{
	log("on_msg_fn_toggle_options_persist_data_get_callback . outlookmail:" + data )
	var enable = get_settings_enable();
	if(!!enable)
	{
		if(g_pagmode)
