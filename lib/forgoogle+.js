...
var fnaddon_com = require("./fnaddon_com.js");
//const 
//pref :
const c_pref_fn_conversation_enable = "fn-googleplus-enable";
//pagemod using 
const c_pagemode_include_expression = "https://plus.google.com/*";
//msgs
...
{
	//this from  bind({worker:aworker, id:data_obj.id})
	aworker.port.emit(c_msg_name_get_composer, aworker.url);
}

//data_recipients = [recipientid, recipientid1,recipientid2] is the data
function on_fnlogic_aHJContentID_recipients_query_feedback(data)
{
	//this from  bind({worker:aworker, id:data_obj.id})
	var aworker = this.worker;	
	var a_id = this.id;
	var data_send = { id:a_id, recipients:data };
	var data_string =  JSON.stringify(data_send);
	aworker.port.emit(c_msg_name_status_fnlogic_aHJContentID_recipients_reply, data_string);
}


function on_fnlogic_aHJContentID_recipients_query(data)
{
	//this = aworker, by bind
	var aworker = this;
	var data_obj =  JSON.parse(data);
	//var data_obj = {HJContentID:aHJContentID, id: a_id };
	fnaddon_com.get_recipients_from_fndocid(data_obj.HJContentID, on_fnlogic_aHJContentID_recipients_query_feedback.bind({worker:aworker, id:data_obj.id}));
}
...
function on_apage_attached(worker)// on page loaded
{
	try
	{
		...
		//addlisteners
		aworker.port.on(c_msg_name_status_fnlogic_aHJContentID_recipients_query,  on_fnlogic_aHJContentID_recipients_query.bind(aworker));
